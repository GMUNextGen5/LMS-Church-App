...
function progressRingSvg(percent: number): string {
  ...
  return `<div class="relative w-28 h-28 shrink-0">
    <svg class="w-28 h-28 -rotate-90" viewBox="0 0 120 120" aria-hidden="true">
      <circle cx="60" cy="60" r="${r}" fill="none" class="stroke-slate-200 dark:stroke-slate-700" stroke-width="10"/>
      <circle cx="60" cy="60" r="${r}" fill="none" class="stroke-cyan-500 dark:stroke-cyan-400" stroke-width="10" stroke-linecap="round"
        stroke-dasharray="${dash} ${c}"/>
    </svg>
    <div class="absolute inset-0 flex items-center justify-center flex-col pointer-events-none">
      <span class="text-2xl font-bold text-on-surface">${p}%</span>
      <span class="text-[10px] text-on-surface-muted font-semibold tracking-wider">GRADED</span>
    </div>
  </div>`;
}
...
function gradeCardHtml(g: Grade, canEdit: boolean): string {
  ...
  const badgePending = pending
    ? `<span class="inline-flex px-2 py-0.5 rounded-full text-xs font-bold border-2 border-cyan-500/70 text-cyan-800 dark:border-cyan-400/80 dark:text-cyan-200">${esc(letter)} (${esc(pctLabel)}%)</span>`
    : `<span class="inline-flex px-2 py-0.5 rounded-full text-xs font-bold bg-cyan-500/15 text-cyan-800 dark:bg-cyan-500/20 dark:text-cyan-300">${esc(letter)} (${esc(pctLabel)}%)</span>`;

  const letters = ['A', 'B', 'C', 'D', 'F']
    .map(
      (L) =>
        `<button type="button" data-grade-quick="${esc(g.id)}" data-letter="${L}" class="min-w-[48px] min-h-[48px] rounded-xl border-2 border-surface-default bg-surface-glass text-on-surface font-bold hover:border-cyan-400/50 transition-colors dark:border-slate-600 dark:bg-slate-800 dark:text-white ${pending && pendingEdits.get(g.id)?.letter === L ? 'ring-2 ring-cyan-400 bg-cyan-500/15' : ''}">${esc(L)}</button>`
    )
    .join('');
  ...
  const actions = canEdit
    ? `<div class="flex flex-col gap-2 mt-3">
        ...
        <div class="flex flex-wrap gap-2">
          <button type="button" data-grade-edit="${esc(g.id)}" class="min-h-[48px] flex-1 rounded-xl bg-surface-glass border border-surface-default text-cyan-800 text-sm font-semibold hover:bg-surface-hover dark:bg-slate-800 dark:border-slate-600 dark:text-cyan-300 dark:hover:bg-slate-700">Edit grade</button>
          <button type="button" data-grade-comment="${esc(g.id)}" class="min-h-[48px] flex-1 rounded-xl bg-surface-glass border border-surface-default text-on-surface-muted text-sm font-semibold hover:bg-surface-hover dark:bg-slate-800 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-700">Add comment</button>
        </div>
        <button type="button" data-grade-absent="${esc(g.id)}" class="w-full min-h-[48px] rounded-xl border-2 border-rose-500/50 text-rose-700 text-sm font-semibold hover:bg-rose-500/10 dark:text-rose-300">Mark absent (today)</button>
      </div>`
    : '';

  return `<article class="rounded-xl border border-surface-default bg-surface-container p-4 shadow-md shadow-slate-200/40 dark:border-slate-600/80 dark:bg-slate-900/50 dark:shadow-lg dark:shadow-black/20" data-grade-row="${esc(g.id)}">
    <div class="flex justify-between gap-2 items-start">
      <div class="min-w-0">
        <h4 class="text-on-surface font-bold truncate">${esc(safeAssignmentTitle(g.assignmentName))}</h4>
        <p class="text-on-surface-muted text-xs mt-1">${esc(g.category)} · ${esc(g.date.slice(0, 10))}</p>
        <p class="text-on-surface-muted text-[11px] font-semibold tracking-wide mt-2">LAST ACTIVITY <span class="text-on-surface">${relativeActivity(g.date)}</span></p>
      </div>
      ${badgePending}
    </div>
    ${actions}
...
export function renderGradesMobile(
  ...
  if (visible.length === 0) {
    ...
    const block = emptyStateBlockHtml(
      ...
      `<button type="button" id="grades-mobile-clear-filter" class="px-4 py-2 rounded-lg border border-surface-default bg-surface-glass text-on-surface text-sm font-semibold hover:bg-surface-hover transition-colors dark:bg-slate-700 dark:text-white dark:hover:bg-slate-600">Show all assignments</button>`
    );
    renderTemplate(cardsRoot, block);
  ...
  renderTemplate(
    ...
    `<div class="flex flex-wrap items-center gap-6">
      ...
      <div class="space-y-1 min-w-0">
        <p class="text-xs font-semibold text-cyan-700 dark:text-cyan-400/90 tracking-widest">GRADING PROGRESS</p>
        <p class="text-on-surface font-semibold text-sm truncate">${filterVal ? esc(filterVal) : 'All assignments'}</p>
        <p class="text-on-surface-muted text-sm">Graded <span class="text-on-surface font-bold">${total - dirty}</span> / ${total}</p>
        <p class="text-on-surface-muted text-sm">Avg score <span class="text-cyan-700 dark:text-cyan-300 font-bold">${avg}%</span></p>
      </div>
    </div>`
  );
